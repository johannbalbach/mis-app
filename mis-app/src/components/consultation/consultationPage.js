...
const ConsultationPage = () => {
    ...
    useEffect(() => {
        const fetchInspections = async () => {
            setLoading(true);
            const data = await consultationsApi.getConsultationsList({ ...filters, grouped: groupByRepeats, page: pagination.current, size: pagination.size }, id);
    ...
    return (
        <Card style={{ width: '80vw', maxWidth: 1600, minWidth: 800, margin: 'auto', marginTop: 30, borderWidth: 0 }}>
            <Row gutter={16} style={{ margin: 'auto' }}>
                <Col span={12}>
                    <Title level={3} style={{ fontWeight: 'bold', fontSize: '2.3rem', textWrap: 'nowrap' }}>Консультации</Title>
                </Col>
            </Row>
